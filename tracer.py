...
class Tracer(object):
	...
	def login(self):
		try:
			...
			if r.status_code == requests.codes.ok:
				...
				logger.error("Couldnt log in. Status Code: {}".format(r.status_code))
	...
	def search(self):
		for c in self.companies:
			try:
				logger.info('Searching for {} cases for the past {} days'.format(c, self.num_days))
				...
				if r.status_code == requests.codes.ok:
					...
					logger.error("Couldnt search for {}. Status Code: {}".format(c, r.status_code))
			...
			for i, table in enumerate(tables):
				...
				for row in table.findAll('tr'):
					...
					if data['line_no']:
						...
						if len(dates) == 2:
							data['date_filed'], data['date_closed'] = [d.text.strip() for d in dates]
